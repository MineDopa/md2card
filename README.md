...
[在线使用](https://haodongcui.github.io/md2card/) · [备用站点](https://md2card.cuihd.com/) · [打开工作台](https://haodongcui.github.io/md2card/app/) · [查看完整样例](./examples/Md2Card%20Example.md)

![Md2Card 产品预览](./public/og-cover.png?v=2)
...
打开 [Md2Card 工作台](https://haodongcui.github.io/md2card/app/)，粘贴或导入一篇 Markdown 笔记，把它整理成一组可直接发布的小红书图片。
...
样例覆盖 H1 / H2 / H3、中文与英文混排、三张本地图片、行内与展示公式、宽表、长表、Python / TypeScript 代码、嵌套列表、长 URL、引用和显式分页。详见 [示例说明](./examples/README.md)。
...
- [x] 手动分页标记 `<!-- md2card:break -->`，而 `---` 保持 Markdown 分隔线语义；卡片末尾的 `---` 会自动隐藏。
- [x] 单独一行写 `<br>` 插入一个空行，连写多个可累加（上限 8 行）；行内 `<br>` 仍是换行。
...
| 分隔线 | `---`；位于卡片末尾时自动隐藏，不会在页脚上方留下孤线 |
| 空行 | 单独一行 `<br>`；行内 `<br>` 作为换行 |

为保证导出安全与一致，除 `<br>` 之外的原始 HTML 会被忽略。内联图片、复杂 HTML 混排、脚注和 Mermaid 暂不属于稳定支持范围；复杂宽表和极长公式建议优先拆分内容或使用 2:3 长内容画布。
...
仓库自带 [GitHub Pages 部署工作流](./.github/workflows/deploy.yml)。推送到 `main` 时会使用 Node.js 22，依次安装依赖、运行测试、构建 `dist/` 并部署。首次部署时，在仓库 **Settings → Pages** 将 Source 设为 **GitHub Actions**。
...
视觉改动还应使用 [完整样例](./examples/Md2Card%20Example.md)，在 Chrome / Edge 中人工检查表格、长公式、多级标题、代码和三张本地图片的预览与导出。
